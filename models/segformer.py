...
class MiT(nn.Module):
    ...
    def forward(self, x):  # z is initialized x 352 352 3
        b = x.shape[0]
        # stage 1
        x, h, w = self.patch_embed1(x)  # 88 88 32
        # print(z.shape)
        # exit()
...
class UnetDecoder(nn.Module):
    def __init__(self, embed_dims, img_size_reductions):
        super().__init__()
    ...
    def forward(self, xs):
        ...
        for i in range(len(self.blocks)):
            if i == 0:
                ...
                x = self.up_sample(x)
                if x.shape[-2] != xs[i].shape[-2] or x.shape[-1] != xs[i].shape[-1]:
                    x = F.interpolate(x, (xs[i].shape[-2], xs[i].shape[-1]), mode='bilinear', align_corners=True)
                x = self.blocks[i](torch.cat([x, xs[i]], dim=1))
...
class SegFormer(BaseModel):
    def __init__(self, backbone: str = 'MiT-B0', num_classes: int = 19) -> None:
        ...
        self.last_layer = nn.Conv2d(self.backbone.embed_dims[0], self.num_classes, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        y = self.last_layer(y)
        y = F.interpolate(y, (x.shape[-2], x.shape[-1]), mode='bilinear', align_corners=True)
        return y


if __name__ == "__main__":
    model = SegFormer('MiT-B0', 1)
